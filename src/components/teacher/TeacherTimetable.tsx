...
export const TeacherTimetable: React.FC = () => {
  ...
  const { teachers, timetable } = useSchool();
  ...
  const mySlots = timetable.filter(t => t.teacherName.includes(currentTeacher?.name || ''));

  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h2 className="text-xl font-black text-slate-900 tracking-tight">Personal Teaching Schedule</h2>
        <p className="text-xs text-slate-500 mt-0.5">
          Weekly timetable and period assignments for {currentTeacher?.name}
        </p>
      </div>
                        ...
                        {slot ? (
                          <div className="p-2.5 rounded-xl bg-amber-500/10 border border-amber-500/20 text-left space-y-0.5">
                            ...
                            <div className="text-[10px] text-amber-800 font-bold">
                              Class {slot.classNumber}-{slot.sectionName}
                            </div>
