...
const SchoolAppInner: React.FC = () => {
    ...
    return (
      <div className="min-h-screen w-full max-w-full overflow-x-hidden bg-slate-50 flex flex-col justify-between selection:bg-amber-500 selection:text-slate-950">
        ...
        <main className="flex-1 w-full max-w-full overflow-x-hidden">
          {/* Mobile-optimized Content Container Wrapper with Consistent Horizontal Spacing */}
          <div className="w-full max-w-full overflow-x-hidden">
            {activePublicView === 'home' ? (
              ...
              <div className="w-full max-w-7xl mx-auto px-3.5 sm:px-6 lg:px-8 py-3.5 sm:py-6">
                {activePublicView === 'about' && <AboutPage onNavigate={handleNavigatePage} />}
  ...
  return (
    <div className="min-h-screen w-full max-w-full overflow-x-hidden bg-[#F6F8FC] flex flex-col selection:bg-indigo-500 selection:text-white">
      {/* Portal Top Bar - Clean Enterprise Admin Masthead */}
      <header className="bg-[#0F172A] text-white border-b border-slate-800 sticky top-0 z-30 px-3 sm:px-6 py-2.5 flex items-center justify-between shadow-xs">
        {/* Left Side: School Branding, Drawer Toggle & Live Breadcrumb */}
        <div className="flex items-center gap-2 sm:gap-3 min-w-0">
          <button
            onClick={() => setIsSidebarOpen(!isSidebarOpen)}
            className="lg:hidden min-h-[44px] min-w-[44px] flex items-center justify-center p-2 rounded-xl bg-slate-800 hover:bg-slate-700 active:bg-slate-600 text-slate-200 transition-all shrink-0 cursor-pointer touch-manipulation active:scale-95"
            aria-label="Toggle Navigation"
            id="btn-portal-drawer-toggle"
          >
            {isSidebarOpen ? <X className="w-5 h-5 text-white" /> : <Menu className="w-5 h-5 text-white" />}
          </button>

          {/* Portal Quick Back Button */}
          {(tabHistory.length > 0 || currentActiveTab !== 'dashboard') && (
            <button
              onClick={handleGoBackTab}
              className="min-h-[40px] flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 hover:border-slate-600 text-xs font-semibold transition-all cursor-pointer shrink-0 group touch-manipulation active:scale-95"
          ...
          <div className="flex items-center gap-3 min-w-0">
            <div className="w-9 h-9 rounded-xl bg-indigo-600 flex items-center justify-center font-bold text-white text-sm shadow-xs shrink-0 border border-indigo-500/40">
              <Building2 className="w-5 h-5 text-white" />
            </div>
            
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                <span className="font-bold text-xs sm:text-sm text-white truncate max-w-[170px] sm:max-w-xs md:max-w-md">
                  {settings.schoolName}
                </span>
                <span className="hidden md:inline-flex px-2 py-0.5 rounded-md bg-indigo-500/20 text-indigo-300 text-[10px] font-semibold border border-indigo-500/30">
                  {role === 'admin' ? 'Headmaster Directorate' : role === 'teacher' ? 'Faculty Directorate' : 'Student Portal'}
                </span>
              </div>
              
              {/* Dynamic Breadcrumb */}
              <div className="text-[11px] text-slate-400 font-normal truncate flex items-center gap-1.5 mt-0.5">
                <span className="text-slate-300 font-medium">{activeTabInfo.pillar}</span>
                <span>›</span>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        {/* Content Canvas */}
        <main className="flex-1 overflow-y-auto px-3.5 sm:px-6 lg:px-8 py-4 sm:py-6 custom-scrollbar w-full max-w-full overflow-x-hidden">
          <div className="max-w-7xl mx-auto w-full">
            ...
            {currentActiveTab !== 'dashboard' && (
              <div className="mb-5 flex flex-wrap items-center justify-between gap-3 bg-white px-4 py-3 rounded-xl border border-slate-200 shadow-xs">
                <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
                  <button
                    onClick={handleGoBackTab}
                    className="min-h-[38px] flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-900 hover:bg-slate-800 text-white text-xs font-semibold transition-all shadow-xs cursor-pointer group shrink-0 touch-manipulation active:scale-95"
                  ...
                  <div className="flex items-center gap-1.5 text-xs text-slate-500 font-normal truncate">
                    <button 
                      onClick={() => handleTabChange('dashboard')}
                      className="min-h-[36px] px-2 py-1 hover:text-indigo-600 active:text-indigo-700 font-medium flex items-center gap-1.5 transition-colors cursor-pointer touch-manipulation rounded-md hover:bg-slate-50"
                    >
                    ...
                    <span className="font-semibold text-slate-900 bg-indigo-50 text-indigo-700 px-2 py-1 rounded-md border border-indigo-100 inline-flex items-center">
                      {language === 'hi' ? activeTabInfo.titleHi : activeTabInfo.titleEn}
                    </span>
                  </div>
                </div>
                ...
                <button
                  onClick={() => handleTabChange('dashboard')}
                  className="min-h-[38px] text-xs font-semibold text-slate-700 hover:text-slate-900 bg-slate-50 hover:bg-slate-100 px-3 py-1.5 rounded-lg border border-slate-200 transition-all flex items-center justify-center gap-1.5 cursor-pointer ml-auto touch-manipulation active:scale-95"
                >
